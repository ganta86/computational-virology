...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    if (nrow(stats_results) > 0) stat_pvalue_manual(data = stats_results, label = "significance", y.position = 4.3, step.increase = 0.02, size = 6) else NULL +
...
ggsave(paste0(save_path, "ADCP_EBNA_violin_dfgeLLOD.pdf"), 
       plot = final_plot1, 
       device = "pdf", 
