...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    coord_cartesian(ylim = c(1e1, 1e7)) +
    theme_classic() +
    labs(
      title = custom_labels[[response_var]],
      x = "EBV infection status",
      y = 'IgG Mean Fluorescence Units'
    ) +
    labssizes + groups + mycolor + myfill +
    # Only add p-value annotations if there are significant results
    if (nrow(stats_results) > 0) stat_pvalue_manual(data = stats_results, label = "significance", y.position = 5.5, step.increase = 0.05, size = 6) else NULL +
...
ggsave(paste0(save_path, "IgG_Cross.pdf"), 
       plot = final_plot1, 
       device = "pdf", 
       dpi = 1000, 
       width = 8, 
