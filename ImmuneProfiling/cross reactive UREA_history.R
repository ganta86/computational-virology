...
create_violin_plot <- function(data, variable, title) {
  ...
  ggplot(filtered_data, aes(x = History, y = value, fill = treatment)) +
    ...
    scale_y_continuous(trans = "log10", limits = c(1e1, 1e5)) +
    theme_classic() +
...
ggsave(paste0(save_path, "IgG_cross_urea_history.pdf"), 
       plot = plot, 
       device = "pdf", 
       dpi = 1000, 
       width = 12, 
