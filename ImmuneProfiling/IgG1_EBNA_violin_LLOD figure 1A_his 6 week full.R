...
perform_paired_tests_with_LLOD <- function(variables_to_test) {
  ...
  for (variable in variables_to_test) {
    ...
    perform_independent_tests_with_LLOD <- function(variables_to_test) {
      # Define independent comparisons
      independent_comparisons <- list(
        c("Acute (n = 97)", "1 year (n = 67)"),
        c("Acute (n = 97)", "6 weeks (n = 67)"),
        c("Acute (n = 97)", "6 months (n = 30)"),
        c("6 weeks (n = 67)", "1 year (n = 67)"),
        c("6 weeks (n = 67)", "6 months (n = 30)"),
        c("6 months (n = 30)", "1 year (n = 67)")
      )
      ...
      for (variable in variables_to_test) {
        for (comparison in independent_comparisons) {
          ...
          # Filter dataset for only the two groups being compared
          subset_data <- COMPILED_DATA_EBNA1b_with_LLOD %>%
            filter(History %in% c(group1, group2)) %>%
            select(History, all_of(variable))
          
          # Check if enough non-missing data exists before testing
          if (nrow(subset_data) > 0 && length(unique(subset_data$History)) == 2) {
            independent_test <- wilcox.test(variable ~ History, data = subset_data)
            ...
            message(paste("Skipping", variable, "comparison:", group1, "vs", group2, "due to insufficient data."))
          }
        }
      }
    }
