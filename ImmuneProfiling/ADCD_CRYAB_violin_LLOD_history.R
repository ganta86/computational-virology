...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    labs(
      ...
      y = 'Complement Deposition(MFI)'
    ) +
    labssizes + groups + mycolor + myfill +
    # Only add p-value annotations if there are significant results
    if (nrow(stats_results) > 0) stat_pvalue_manual(data = stats_results, label = "significance", y.position = 6, step.increase = 0.05, size = 6) else NULL +
...
ggsave(paste0(save_path, "ADCD_Cross_history.pdf"), 
       plot = final_plot3, 
       device = "pdf", 
       dpi = 1000, 
       width = 8, 
