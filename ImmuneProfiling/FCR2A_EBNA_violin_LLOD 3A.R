...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    labs(
      ...
      y = 'FCRIIa Mean Fluorescence Units'
    ) +
    labssizes + groups + mycolor + myfill +
    # Only add p-value annotations if there are significant results
    if (nrow(stats_results) > 0) stat_pvalue_manual(data = stats_results, label = "significance", y.position = 6.7, step.increase = 0.04, size = 6) else NULL +
...
ggsave(paste0(save_path, "FcγRIIa_plots.pdf"), 
       plot = final_plot2, 
       device = "pdf", 
       dpi = 1000, 
       width = 12, 
