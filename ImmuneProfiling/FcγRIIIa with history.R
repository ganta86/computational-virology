...
# Define groups and colors
groups <- scale_x_discrete(limits=c("Acute (n = 97)", "6 weeks (n = 67)", "6 months (n = 30)", "1 year (n = 67)", "Sero_Pos (n = 30)", "Sero_Pos (n = 20)_HX", "Sero_Negative"))
mycolor <- scale_color_manual(breaks = c("Acute (n = 97)", "6 weeks (n = 67)", "6 months (n = 30)", "1 year (n = 67)", "Sero_Pos (n = 30)", "Sero_Pos (n = 20)_HX", "Sero_Negative"), 
                              values = c("magenta4", "dodgerblue3", "chocolate3", "darkgreen", "blue", "yellow4", "black"))
myfill <- scale_fill_manual(breaks = c("Acute (n = 97)", "6 weeks (n = 67)", "6 months (n = 30)", "1 year (n = 67)", "Sero_Pos (n = 30)", "Sero_Pos (n = 20)_HX", "Sero_Negative"), 
                            values = c("magenta4", "dodgerblue3", "chocolate3", "darkgreen", "blue", "yellow4", "black"))
# Create a named vector for custom labels (updated with the new variables)
custom_labels <- c(
  "FcγRIIIa_EBNA1_421_476_B958" = "aa 421-476_B958",
  "FcγRIIIa_EBNA1_589_641_B958" = "aa 589-641_B958",
  "FcγRIIIa_EBNA1_589_641_AG876" = "aa 589-641_AG876",
  "FcγRIIIa_EBNA1_1_56_GD1" = "aa 1-56_GD1",
  "FcγRIIIa_EBNA1_1_56_B958" = "aa 1-56_B958",
  "FcγRIIIa_EBNA1_421_476_AG876" = "aa 421-476_AG876",
  "FcγRIIIa_EBNA1_449_504_B958" = "aa 449-504_B958",
  "FcγRIIIa_EBNA1_1_90" = "aa 1-90",
  "FcγRIIIa_EBNA1_377_459" = "aa 377-459",
  "FcγRIIIa_EBNA1_460_641" = "aa 460-641",
  "FcγRIIIa_EBNA1_29_84_GD1" = "aa 29-84_GD1",
  "FcγRIIIa_EBNA1_365_420_B958" = "aa 365-420",
  "FcγRIIIa_EBNA1_393_448_B958" = "aa 393-448",
  "FcγRIIIa_EBNA1_393_448_AG876" = "aa 393-448_AG876",
  "FcγRIIIa_GP350_R" = "GP350_R",
  "FcγRIIIa_VCA_p23" = "VCA_p23",
  "FcγRIIIa_Early_Ag" = "Early_Ag",
  "FcγRIIIa_GH" = "GH",
  "FcγRIIIa_GP350_V" = "GP350_V"
)

# List of variables to test (updated with new variables)
variables_to_test <- c("FcγRIIIa_EBNA1_421_476_B958", "FcγRIIIa_EBNA1_589_641_B958", "FcγRIIIa_EBNA1_589_641_AG876", 
                       "FcγRIIIa_EBNA1_1_56_GD1", "FcγRIIIa_EBNA1_1_56_B958", "FcγRIIIa_EBNA1_421_476_AG876", 
                       "FcγRIIIa_EBNA1_449_504_B958", "FcγRIIIa_EBNA1_1_90", "FcγRIIIa_EBNA1_377_459", 
                       "FcγRIIIa_EBNA1_460_641", "FcγRIIIa_EBNA1_29_84_GD1", "FcγRIIIa_EBNA1_365_420_B958", 
                       "FcγRIIIa_EBNA1_393_448_B958", "FcγRIIIa_EBNA1_393_448_AG876", "FcγRIIIa_GP350_R", 
                       "FcγRIIIa_VCA_p23", "FcγRIIIa_Early_Ag", "FcγRIIIa_GH", "FcγRIIIa_GP350_V")
...
independent_comparisons <- list(
  ...
  c("Sero_Pos (n = 30)", "Sero_Negative"),
  c("Sero_Pos (n = 20)_HX", "Sero_Negative"),
  c("Sero_Pos (n = 20)_HX", "1 year (n = 67)"),
  c("Sero_Pos (n = 30)", "1 year (n = 67)"),
  c("Acute (n = 97)", "Sero_Pos (n = 30)"),
  c("Acute (n = 97)", "Sero_Pos (n = 20)_HX"),
  c("Acute (n = 97)", "Sero_Negative"),
  c("Sero_Pos (n = 30)", "Sero_Pos (n = 20)_HX")
)
...
perform_paired_tests_with_LLOD <- function(variables_to_test) {
  ...
  paired_data <- COMPILED_DATA_EBNA1b_with_LLOD %>%
    filter((History == "Acute (n = 97)" & visit %in% c("v1", "v2")) | 
             (History == "1 year (n = 67)" & visit == "v8") |
             (History == "6 months (n = 30)" & visit == "v7")) %>%
    pivot_wider(
...
perform_independent_tests_with_LLOD <- function(variables_to_test) {
  for (variable in variables_to_test) {
    for (comparison in independent_comparisons) {
      subset_data <- COMPILED_DATA_EBNA1b_with_LLOD %>% filter(History %in% comparison)
      independent_test <- wilcox.test(get(variable) ~ History, data = subset_data)
      all_p_values <<- rbind(all_p_values, data.frame(
        response_var = variable,
...
# Filter out non-significant results for plotting
...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    labs(
      ...
      y = 'FcγRIIIa Mean Fluorescence Units'
    ) +
    labssizes + groups + mycolor + myfill +
    # Only add p-value annotations if there are significant results
    if (nrow(stats_results) > 0) stat_pvalue_manual(data = stats_results, label = "significance", y.position = 5.9, step.increase = 0.05, size = 6) else NULL +
...
# Combine specific plots using patchwork
final_plot1 <- plots[[4]] | plots[[5]] | plots[[8]] | plots[[11]]
final_plot2 <- plots[[12]]| plots[[9]] | plots[[13]]
final_plot3 <- plots[[1]] | plots[[6]] | plots[[2]] | plots[[3]]
final_plot4 <- plots[[7]] | plots[[10]]
...
ggsave(paste0(save_path, "FcγRIIIa_EBNA_violin_dfgeLLOD_history.pdf"), 
       plot = final_plot2, 
       device = "pdf", 
...
calculate_percentage_positive <- function(data, variables, LLOD_values, group_var = "History") {
  results <- data.frame()
