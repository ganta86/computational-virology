...
variables_to_test <- c("IgG1_EBNA1_421_476_B958", "IgG1_EBNA1_589_641_B958", "IgG1_EBNA1_589_641_AG876", 
...
independent_comparisons <- list(
  ...
  c("6 weeks (n = 67)", "Sero_Negative"),  # Added comparison
  c("6 weeks (n = 67)", "1 year (n = 67)"),  # Added comparison
  c("6 weeks (n = 67)", "Sero_Pos (n = 30)"),  # Added comparison
  c("6 weeks (n = 67)", "Acute (n = 97)"),
  c("6 weeks (n = 67)", "Sero_Pos (n = 20)_HX")# Added comparison
)

# Function to perform paired tests, now including "6 weeks (n = 67)"
perform_paired_tests_with_LLOD <- function(variables_to_test) {
  paired_data <- COMPILED_DATA_EBNA1b_with_LLOD %>%
    filter((History == "Acute (n = 97)" & visit %in% c("v1", "v2")) | 
             ...
             (History == "6 weeks (n = 67)" & visit %in% c("v5", "v6"))) %>%
    pivot_wider(
  ...
  print("✅ Paired Data Created!")
  print(dim(paired_data))  # Debugging step
  print(colnames(paired_data))  # Check if all groups exist
  ...
  for (variable in variables_to_test) {
    ...
    six_weeks_col <- paste0(variable, "_6 weeks (n = 67)")
    
    # ✅ Acute vs 1 Year
    if (acute_col %in% colnames(paired_data) && one_year_col %in% colnames(paired_data)) {
      complete_pairs_1year <- paired_data %>%
        filter(!is.na(.[[acute_col]]) & !is.na(.[[one_year_col]]))
      
      if (nrow(complete_pairs_1year) > 0) {
        paired_test_1year <- wilcox.test(
        ...
        all_p_values <<- rbind(all_p_values, data.frame(
          ...
          p_value = paired_test_1year$p.value,
          group1 = "Acute (n = 97)",
          group2 = "1 year (n = 67)",
          test_type = "Paired"
        ))
    ...
    # ✅ Acute vs 6 Months
    if (acute_col %in% colnames(paired_data) && six_months_col %in% colnames(paired_data)) {
      complete_pairs_6months <- paired_data %>%
        filter(!is.na(.[[acute_col]]) & !is.na(.[[six_months_col]]))
      
      if (nrow(complete_pairs_6months) > 0) {
    ...
    # ✅ Acute vs 6 Weeks
    if (acute_col %in% colnames(paired_data) && six_weeks_col %in% colnames(paired_data)) {
      complete_pairs_6weeks <- paired_data %>%
        filter(!is.na(.[[acute_col]]) & !is.na(.[[six_weeks_col]]))
      
      if (nrow(complete_pairs_6weeks) > 0) {
        paired_test_6weeks <- wilcox.test(
          complete_pairs_6weeks[[acute_col]], 
          complete_pairs_6weeks[[six_weeks_col]], 
          paired = TRUE
        )
        all_p_values <<- rbind(all_p_values, data.frame(
          response_var = variable,
          comparison = "Acute_vs_6Weeks",
          p_value = paired_test_6weeks$p.value,
          group1 = "Acute (n = 97)",
          group2 = "6 weeks (n = 67)",
          test_type = "Paired"
        ))
...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    labs(
      ...
      y = 'IgG1 Mean Fluorescence Units'
    ) +
...
ggsave(paste0(save_path, "IgG1_EBNA_violin_dfgeLLOD_history.pdf"), 
       plot = final_plot2, 
       device = "pdf", 
