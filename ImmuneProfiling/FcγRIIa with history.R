...
variables_to_test <- c("FcγRIIa_EBNA1_421_476_B958", "FcγRIIa_EBNA1_589_641_B958", "FcγRIIa_EBNA1_589_641_AG876", 
...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    coord_cartesian(ylim = c(1e3, 1e9)) +
    theme_classic() +
    labs(
      title = custom_labels[[response_var]],
      x = "EBV infection status",
      y = 'FcγRIIa Mean Fluorescence Units'
    ) +
...
ggsave(paste0(save_path, "FcγRIIa_EBNA_violin_dfgeLLOD_history.pdf"), 
       plot = final_plot2, 
       device = "pdf", 
