...
custom_labels <- c(
  "ADCP_EBNA1_365_420_B958"= "aa 365-420",
  "ADCP_EBNA1_377_459"= "aa 377-459",
  "ADCP_EBNA1_393_448_AG876"= "aa 393-448"
)

# List of variables to test (updated with new variables)
variables_to_test <- c("ADCP_EBNA1_365_420_B958",
                       "ADCP_EBNA1_377_459",
                       "ADCP_EBNA1_393_448_AG876")
...
create_plot <- function(df, response_var, group_var, stats_results) {
  ggplot(df, aes_string(x = group_var, y = response_var)) +
    ...
    coord_cartesian(ylim = c(10^3.3, 10^4.7)) +
    theme_classic() +
    labs(
    ...
    if (nrow(stats_results) > 0) stat_pvalue_manual(data = stats_results, label = "significance", y.position = 4.3, step.increase = 0.01, size = 6) else NULL +
...
ggsave(paste0(save_path, "ADCP_EBNA_violin_dfgeLLOD_history.pdf"), 
       plot = final_plot1, 
       device = "pdf", 
